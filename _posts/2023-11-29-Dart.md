...
- ***".join"*** = .join bir iterable'ın (liste, dize vb.) elemanlarını belirli bir ayraç veya dize kullanarak birleştirip tek bir dize haline getirir. Bu metot, iterable'ın elemanlarını birleştirirken aralarına isteğe bağlı olarak bir karakter veya dize eklemek için kullanılır. Örneğin:
